...
/* Send buffer node for pending writes */
...
static void queue_send(ev_ws_conn_t* conn, const char* data, size_t len, enum lws_write_protocol write_mode) {
    ...
    Newx(send, 1, ev_ws_send_t);
    /* Allocate with LWS_PRE padding for libwebsockets */
    Newx(send->data, LWS_PRE + len, char);
    if (data && len > 0) {
...
static ev_ws_fd_t* find_fd_watcher(ev_ws_ctx_t* ctx, int fd) {
    ev_ws_fd_t* fdw;
    for (fdw = ctx->fd_watchers; fdw != NULL; fdw = fdw->next) {
        if (fdw->fd == fd) {
            return fdw;
        }
    }
...
static void add_fd_watcher(ev_ws_ctx_t* ctx, int fd, int events) {
    ...
    /* Add to list */
    fdw->next = ctx->fd_watchers;
    if (ctx->fd_watchers) {
        ctx->fd_watchers->prev = fdw;
    }
    ctx->fd_watchers = fdw;
}

static void del_fd_watcher(ev_ws_ctx_t* ctx, int fd) {
    ev_ws_fd_t* fdw = find_fd_watcher(ctx, fd);
    if (fdw == NULL) return;
    ...
    /* Remove from list */
    if (fdw->prev) {
        fdw->prev->next = fdw->next;
    } else {
        ctx->fd_watchers = fdw->next;
    }
    if (fdw->next) {
        fdw->next->prev = fdw->prev;
    }
...
static void free_all_fd_watchers(ev_ws_ctx_t* ctx) {
    ...
    for (fdw = ctx->fd_watchers; fdw != NULL; fdw = next) {
        next = fdw->next;
        if (ctx->loop) {
            ev_io_stop(ctx->loop, &fdw->io);
        }
        ...
        case LWS_CALLBACK_CLIENT_APPEND_HANDSHAKE_HEADER: {
            if (conn && conn->custom_headers) {
                ...
                while ((entry = hv_iternext(hv))) {
                    ...
                    /* Append colon to key for lws */
                    char *key_colon;
                    Newx(key_colon, klen + 2, char);
                    
                    memcpy(key_colon, key, klen);
                    key_colon[klen] = ':';
                    key_colon[klen+1] = '\0';
                    
                    DEBUG_LOG("Adding header: %s %s", key_colon, val);
                    
                    if (lws_add_http_header_by_name(wsi, (unsigned char*)key_colon, (unsigned char*)val, vlen, p, end)) {
                        Safefree(key_colon);
                        return -1;
                    }
        ...
        case LWS_CALLBACK_ADD_HEADERS: {
            ...
            if (srv && srv->magic == EV_WS_SRV_MAGIC && srv->response_headers) {
                ...
                while ((entry = hv_iternext(srv->response_headers))) {
                    ...
                    key_colon[klen] = ':';
                    key_colon[klen + 1] = '\0';
                    int rc = lws_add_http_header_by_name(wsi, (unsigned char *)key_colon,
...
CODE:
{
    ...
    /* Set up foreign loop for native libev integration */
...
CODE:
{
    ...
    /* Stop service timer and idle watcher */
    ev_timer_stop(self->loop, &self->timer);
    ev_idle_stop(self->loop, &self->idle);

    /* Stop all fd watchers (if any were added via external poll fallback) */
...
CODE:
{
    ...
    /* Parse URL - make a copy we can modify */
    ...
    if (strncasecmp(url_copy, "wss://", 6) == 0) {
        ...
        if (!ssl_verify) {
            use_ssl |= LCCSCF_ALLOW_SELFSIGNED | LCCSCF_SKIP_SERVER_CERT_HOSTNAME_CHECK;
        }
        port = 443;
        host = url_copy + 6;
...
CODE:
{
    ...
    {
        struct lws_vhost *vh = lws_get_vhost_by_name(self->lws_ctx, "default");
        if (!vh) {
            unlink_conn(RETVAL);
            free_conn_resources(RETVAL);
            RETVAL->magic = EV_WS_CONN_FREED;
...
CODE:
{
    ...
    /* Request callback to send close frame */
