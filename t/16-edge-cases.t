...
# 1. Sequential connection reuse after failure
{
    ...
    # First: connect to a port that refuses
    my ($err1, $err2, $got_msg);
    my %keep;

    my $t1 = EV::timer(0.1, 0, sub {
        $keep{c1} = $ctx->connect(
            url => "ws://127.0.0.1:1",  # should refuse
            on_error => sub { $err1 = $_[1]; delete $keep{c1} },
        );
    });

    # Second: connect to our real server (after the first fails)
...
# 2. Server on_connect without storing $_[0]
{
    ...
    my $port = $ctx->listen(
        port => 0,
        on_connect => sub {}, # deliberately NOT storing the connection
        on_message => sub {
            my ($c, $data) = @_;
            $srv_msg = $data;
            $c->send("echo:$data");
...
# 6. Context destroy from inside on_message (alive_flag test)
{
    ...
    my $t = EV::timer(0.1, 0, sub {
        $keep{c} = $ctx->connect(
            ...
            on_message => sub {
                $msg_received = 1;
                # Destroy context from inside callback
                %keep = ();
                undef $ctx;
                $destroyed_ok = 1;
                EV::break;
